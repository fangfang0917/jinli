...
//        $map['level']=['NOT IN',[2,3,4,5]];
    ...
    public function buy()
    {
        ...
        $list = db('order')->alias('o')->join('user u', 'u.id = user_id', 'left')
            ->where(array('user_id' => $id))->field(array('u.nick_name', 'o.order_no', 'o.money', 'o.course_id',
                'o.add_time', 'o.pay_type', 'o.id'))->paginate(10, false, ['query' => request()->param()]);
        $this->view->assign('list', $list);
        $this->view->assign('count', $list->count());
        $this->view->assign('page', $list->render());
        return $this->view->fetch();
