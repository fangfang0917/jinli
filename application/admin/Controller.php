...
class Controller
{
    ...
    public function __construct()
    {
        ...
        $this->view->assign('CCT',Config::get('course_classify_type'));
        // 前置方法
    ...
    protected function total_num($type)
    {
        if ($type == 1) {
            $beginToday = 0;
            $endToday = time();
            ...
            $timestamp = time();
            $beginToday = strtotime(date('Y-m-d', strtotime("this week Monday", $timestamp)));
            $endToday = strtotime(date('Y-m-d', strtotime("this week Sunday", $timestamp))) + 24 * 3600 - 1;
        } elseif ($type == 5) {

        } else {

        }
        $user = Db('user')->field(array('(select count(*) from tp_user where add_time>' . $beginToday . ' and add_time<' . $endToday . ') as total_user_num',
            '(select count(*) from tp_user where level=1 and add_time>' . $beginToday . ' and add_time<' . $endToday . ') as total_vip_user_num',
            '(select count(*) from tp_user where level=2 and add_time>' . $beginToday . ' and add_time<' . $endToday . ') as total_c_user_num',
            '(select count(*) from tp_user where level=3 and add_time>' . $beginToday . ' and add_time<' . $endToday . ') as total_z_user_num',
            '(select count(*) from tp_user where level=4 and add_time>' . $beginToday . ' and add_time<' . $endToday . ') as total_g_user_num',
            '(select count(*) from tp_user where level=5 and add_time>' . $beginToday . ' and add_time<' . $endToday . ') as total_s_user_num'))->find();
        $map = [];
        $map['order_type'] = 1;
        $map['add_time'] = array('between', $beginToday . "," . $endToday);
        $map['money'] = ['gt',0];
        $course_total_num = DB('order')->where($map)->count();
        $course_total_money = DB('order')->where($map)->sum('money');
        return array('user'=>$user,'course_total_num'=>$course_total_num,'course_total_money'=>$course_total_money);
    }
}
