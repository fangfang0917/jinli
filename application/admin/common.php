...
function getUser($userid,$f = 'nick_name'){
    return db('user')->where(array('id'=>$userid))->value($f);
}

function getcourse(){
    $data = db('course')->select();
    return json_encode($data);
}

function getCourseName($id,$f = 'course_title'){
    return Db('course')->where(array('id'=>$id))->value($f);
}

function getlevel($level){
    return  \think\Config::get('levelname')['level'.$level];
}
